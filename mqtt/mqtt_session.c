...
extern void mqtt_subscribe_request(tmq_broker_t* broker, tmq_str_t client_id, tmq_subscribe_pkt* subscribe_pkt);
extern void mqtt_unsubscribe_request(tmq_broker_t* broker, tmq_str_t client_id, tmq_unsubscribe_pkt* unsubscribe_pkt);
                               ...
                               char* will_message, uint8_t will_qos, uint8_t will_retain, uint8_t max_inflight)
{
    ...
    session->conn = get_conn_ref(conn);
...
void tmq_session_handle_subscribe(tmq_session_t* session, tmq_subscribe_pkt* subscribe_pkt)
{
    ...
    mqtt_subscribe_request((tmq_broker_t*) session->upstream, session->client_id, subscribe_pkt);
}
...
void tmq_session_handle_unsubscribe(tmq_session_t* session, tmq_unsubscribe_pkt* unsubscribe_pkt)
{
    ...
    mqtt_unsubscribe_request((tmq_broker_t*) session->upstream, session->client_id,unsubscribe_pkt);
}
