...
int main(int argc, char* argv[])
{
    ...
    tmq_cmd_add_string(&cmd, "pwd", "p", "password", 0, "tinymqtt_pub");
    tmq_cmd_add_string(&cmd, "topic", "t", "topic", 1, "");
    tmq_cmd_add_string(&cmd, "message", "m", "publish message", 1, "");
    tmq_cmd_add_number(&cmd, "qos", "q", "publish qos", 0, 0);
    tmq_cmd_add_bool(&cmd, "retain", "r", "retain");
    ...
    if(tmq_cmd_parse(&cmd, argc, argv) != -1)
    {
        ...
        if(ret == CONNECTION_ACCEPTED)
            tinymqtt_publish(mqtt, tmq_cmd_get_string(&cmd, "topic"), tmq_cmd_get_string(&cmd, "message"),
                             tmq_cmd_get_number(&cmd, "qos"), tmq_cmd_exist(&cmd, "retain"));
        else
